## The Autoload in charge of managing the Music of the game
extends Node

#region Declarations
@export var song_pool : Array[OstHolder] ## The entire collection of OSTs available to use in the game.

var main_music_player : AudioStreamPlayer ## The Primary AudioPlayer of OSTs
var secondary_music_player : AudioStreamPlayer ## A Secondary Player to use with cross fading.
var current_song : OstHolder ## The currently playing song
var crossfade := false ## If [member current_song] is currently cross fading.
#endregion
...
## Plays a song based on the [param song_name] at [param play_position] seconds into the song. [br]
...
func play_song(song_name: StringName, play_position: float = 0, crossover: bool = false) -> void:
	...
	if crossover:
		...
		main_music_player.play(play_position)

## Plays a short jingle from [class SoundManager] by cross fading [member current_song] assuming [br]
...
## Stops [member current_song] from playing assuming it currently is.
func stop_song() -> void:
