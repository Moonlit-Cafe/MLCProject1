...
@export var top_most_tile : BasicTile
#endregion

#region Events
static func generate_column(map: GameTileMap, zone_data: ZoneData, rng: RandomNumberGenerator) -> TileColumn:
	...
	for z in range(height):
		...
		if map.tile_references.has(tile_id):
			...
			new_tile = BasicTile.create_tile(tile_data.texture, tile_data.texture_type)
